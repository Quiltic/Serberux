...
def achual_stuff(putin,Play,Bad,more = False):
    ...
    if Turn > 0:
        ...
        if more != True:
            extra = '.'
            if Play['Strain']+Play['Permeffects']['Strainmax']<= 0:
                Play['Aflictions'].append('Strained')
                extra = " : You are Strained."
            else:
                extra = (' : %s/%s Strain' % (Play['Strain'],Play['MaxStrain']))
                
            Play['Dodge'] = Play['STDodge'] + Play['Permeffects']['Dodge']
            if Play['Hp'] < (Play['MaxHp']+ Play['Permeffects']['HpBoost']/4):
                Play['Dodge'] = Play['STDodge'] + 20 + Play['Permeffects']['Dodge']
        ...
        if Bad['Strain']<= 0:
                Bad['Aflictions'].append('Strained')
        AFLICT(Bad,Play)
...
def AI(Owner,Target):
    ...
    Spells = load("Spells")
    if (Owner['MaxHp']/random.randrange(5,7)) > Owner['Hp']:
...
def Use_Spell(spell,Owner,Target):
    ...
    Fornow = {'Fire':'Ablaze','Stun':'Stuned','Weak':'Weakend','Regenerate':'Regenerating','Bleed':'Bleeding','NA':'','':''}
    if spell[2] > 0:
        dmg = random.randrange(spell[1],spell[2])+Owner['MaxStrain']+Owner['Permeffects']['Strainmax']
...
def DAMAGE(Owner,Target,Type,dmg = 'NA'):
    ...
    if random.randrange(0,Target['Dodge']) < 40:
        if dmg != "NA":
            ...
            dmg = random.randrange(0,Owner['MaxP']+ Owner['Permeffects']['Damage']) - Target['Resist'] - Target['Permeffects']['Resistance']
        if dmg > 0:
...
def AFLICT(Owner,Target):
    ...
    dodgeup = ['Hidden']
    stun = ['Stun']
    dodgedown = ['Blinded','Strained']
    resistancedown = ['Strained']
    resistanceup = ['Shealded']
    dmgup = []
    dmgdown = ['Weak','Strained']
